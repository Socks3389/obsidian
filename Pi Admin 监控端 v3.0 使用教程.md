...
双击 `Pi Monitor.exe` 即可启动程序。首次运行时，Windows Defender 或防火墙可能会提示网络访问请求，请点击“允许访问”。
...
2. **输入 Token**: 将 Token 粘贴到输入框中。

3. **连接服务器**: 点击按钮，验证通过后将进入监控仪表盘。

4. **系统设置**：点击设置按钮，可自行设置采集时间间隔， 操作延迟，开机自启动等相关功能！
> ⚠️*警告*：API地址请根据 Pi Admin 为准，填写错误会导致无法正常使用， 操作延迟请按照建议 5000ms 进行配置，避免因点击等速度过快，Pi客户端响应缓慢，无法正常获取数据！
